...
def findJsonFilePaths(rootPath):
    for parentFolderPath, folderNames, fileNames in os.walk(rootPath.encode("utf-8")):
...
def loadTestList():
    ...
    for jsonFilePath in findJsonFilePaths(module_tests_data_path):
        testPath = common.getFirstPathRelativeToSecondPathIfWithin(jsonFilePath[:-5], module_tests_data_path)
